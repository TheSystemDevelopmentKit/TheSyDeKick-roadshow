...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\renewcommand{\sectionname}{Example5: Outphasing transmitter}
\subsection*{\sectionname}
\begin{frame}[c]
    \frametitle{\sectionname}
    \begin{itemize}
    ...
    \begin{center}
        \begin{tikzpicture}[every node/.style={inner sep=0ex,outer sep=0}]
            \tikzstyle{item}=[draw=black, rounded corners, 
                 fill=secondarycolor!10, drop shadow, align=center, 
                  anchor=center, text=black, rectangle, 
                   minimum width=3cm, minimum height=2cm]
                   \node (soft) [item]{ ELF fiele };
                   \node (chip) [anchor = west, left = 1cm of soft.east ]{\includegraphics[width=.8\textwidth]{./Pics/acorechip.eps} };
                   %draw[>=latex,primarycolor,very thick,<->] (n2.east) 
                   %to [out=0,in=180] (n5.west);
               \end{tikzpicture}
    \end{center}
\end{frame}
